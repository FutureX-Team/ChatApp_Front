...
export default function Tweet({ tweet, currentUser, onReply, onDelete }) {
  ...
  const baseReplyCount = useMemo(() => {
    const direct =
      tweet?.replies_count ?? tweet?.reply_count ?? tweet?.repliesCount ?? tweet?.replies_count_agg ?? null;
          ...
          <div className="flex items-start">
            ...
            <div className="ml-3 w-full cursor-pointer">
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-xs text-gray-500 dark:text-gray-400">{timeAgo(tweet?.created_at)}</span>
              </div>
              {isReply && (
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                  ↪︎ ردًا على{" "}
                  <Link to={`/tweet/${tweet.reply_to_tweet_id}`} className="underline" onClick={(e) => e.stopPropagation()}>
                    هذه التغريدة
                  </Link>
                </div>
              )}
              <p className="mt-2 break-words break-all">{tweet?.text ?? ""}</p>
            </div>
          </div>
              ...
              {dropdownOpen && (
                <div className="absolute -right-[164px] mt-1 w-48 bg-white dark:bg-gray-700 shadow-xl rounded-lg z-20">
                  {canDelete && (
                    <button
                      onClick={() => {
                        if (!window.confirm("هل أنت متأكد من حذف التغريدة؟")) return;
                        setDropdownOpen(false);
