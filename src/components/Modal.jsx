...
<textarea
  value={text}
  onChange={(e) => {
    if (e.target.value.length <= maxChars) setText(e.target.value); // يمنع الكتابة بعد 280 حرف
  }}
  maxLength={maxChars}
...
/>
  ...
  <span
    className={`font-mono ${remainingChars <= 20 ? "text-red-500" : "text-gray-500"}`}
  >
    {remainingChars}
  </span>
<button
  onClick={handleSubmit}
  disabled={!text.trim() || text.length >= maxChars} // ⚡ الزر يتوقف عند 280 حرف
