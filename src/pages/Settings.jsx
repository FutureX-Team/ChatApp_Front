...
export default function Settings({ user: userProp, onLogout, setUser }) {
  ...
  const [password, setPassword] = useState(""); // اختياري
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (v) {
        const flat = Object.fromEntries(Object.entries(v).map(([k, arr]) => [k, Array.isArray(arr) ? arr[0] : String(arr)]));
        setErrors(flat);
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <form onSubmit={handleSave} className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-xl shadow-lg">
        ...
        <div className="mb-6">
          <label htmlFor="password" className="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">كلمة المرور (اختياري)</label>
          <input
            id="password"
            type="password"
        ...
        <div className="mb-6 flex items-center gap-3">
          <input
            ...
            className="h-4 w-4"
          />
          <label htmlFor="darkmode" className="text-sm text-gray-700 dark:text-gray-300">تفعيل الوضع الليلي</label>
        </div>
