...
export default function Settings({ user: userProp, onLogout, setUser, darkMode, setDarkMode }) {
  ...
  // تحميل بيانات المستخدم عند البداية
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
    ...
    <div className="max-w-2xl mx-auto">
      ...
      >
        ...
        <div className="flex items-center mb-6">
          <label className="mr-3 text-gray-700 dark:text-gray-300">الوضع الليلي</label>
          <input
            type="checkbox"
            checked={darkMode}
        ...
        <div className="flex justify-center items-center gap-4 mb-6">
          {avatarUrl ? (
            <img
              ...
              onError={(e) => { e.currentTarget.src = ""; setAvatarUrl(""); }}
            />
          ) : (
            <Avatar name={username || email || "?"} size="h-16 w-16 text-2xl" />
          )}
        </div>

        {/* اسم المستخدم */}
        <div className="mb-4">
          <label className="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">اسم المستخدم</label>
          <input
            type="text"
            value={username}
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">كلمة المرور الحالية</label>
            <button
              type="button"
          ...
          <input
            ...
            className={`w-full p-3 border rounded-lg text-gray-900 dark:text-white border-gray-300 dark:border-gray-600
              ${editingCurrentPassword ? "bg-white dark:bg-gray-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" : "bg-gray-100 dark:bg-gray-700 cursor-not-allowed opacity-70"}`}
          />
        </div>

        {/* كلمة مرور جديدة */}
        <div className="mb-6">
          <label className="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">كلمة المرور الجديدة</label>
          <input
            type="password"
