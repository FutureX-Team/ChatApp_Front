...
export default function TweetDetail({ user }) {
  ...
  const handleAddReply = async (replyData) => {
    ...
    try {
      ...
      if (!newReply.user && newReply?.id) newReply = normalize(await api.get(`/tweets/${newReply.id}`));

      const addReplyToTree = (t) => {
        if (t.id === replyingTo.id) return { ...t, replies_count: (t.replies_count ?? 0) + 1, replies: [newReply, ...(t.replies ?? [])] };
        return { ...t, replies: (t.replies ?? []).map(addReplyToTree) };
      };

      setTweet((prev) => addReplyToTree(prev));
      setShowReplyModal(false); setReplyingTo(null);
  ...
  const handleDelete = (tweetId) => {
    setTweet(prev => {
      const remove = (t) => !t ? null : t.id === tweetId ? null : { ...t, replies: (t.replies ?? []).map(remove).filter(Boolean) };
      return remove(prev);
    });

    api.delete(`/tweets/${tweetId}`).catch(err => showErrorToast(err, "تعذّر حذف التغريدة من السيرفر"));
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <Tweet tweet={tweet} currentUser={user} onReply={() => { setReplyingTo(tweet); setShowReplyModal(true); }} onDelete={handleDelete} />
      {tweet.replies?.map(reply => <Tweet key={reply.id} tweet={reply} currentUser={user} onReply={() => { setReplyingTo(reply); setShowReplyModal(true); }} onDelete={handleDelete} onClick={() => {}} />)}
    </div>
  );
}
